...
# ======== ДОБАВЬТЕ ЭТИ 3 РОУТА ========

@api_bp.route('/categories', methods=['GET', 'POST'])
@login_required
def handle_categories():
    """Обработка категорий (GET и POST)"""
...
def create_web_event():
    """Создать событие из веб-интерфейса"""
    data = request.get_json()
...
@api_bp.route('/events/week/<week_id>', methods=['GET'])
@login_required
def get_week_events(week_id):
